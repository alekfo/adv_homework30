...
def test_create_client_with_factory(client, db):
    ...
    resp = client.post(
        "/clients", data=json.dumps(client_dict), content_type="application/json"
    )
...
def test_create_parking_with_factory(client, db):
    ...
    resp = client.post(
        "/parkings", data=json.dumps(parking_dict), content_type="application/json"
    )
    ...
    created_parking = (
        db.session.query(Parking).filter(Parking.id == response_data["id"]).first()
    )
