...
def create_app():
    ...
    def get_client_by_id(client_id):
        client: Client = db.session.query(Client).filter(Client.id == client_id).one_or_none()
        if client:
    ...
    def create_client_parkings():
        try:
            ...
            client: Client = db.session.query(Client).filter(Client.id == client_id).one_or_none()
            parking: Parking = db.session.query(Parking).filter(Parking.id == parking_id).one_or_none()
            ...
            new_client_parking = ClientsParking(client_id=client_id,
    ...
    def delete_client_parkings():
        ...
        parking_id = data.get('parking_id')
        parking: Parking = db.session.query(Parking).filter(Parking.id == parking_id).one_or_none()
        client_parking: ClientsParking = db.session.query(ClientsParking).filter(ClientsParking.client_id == client_id,
                                                                 ClientsParking.parking_id == parking_id,).one_or_none()
        old_count_available_places = parking.count_available_places
        if not client_parking:
            return jsonify({"error": "Такого бронирования не существует, проверьте данные"}), 404
